...
// GET /api/supermarket-dispositifs/:supermarketId
router.get('/:supermarketId', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM supermarket_dispositifs WHERE supermarket_id = $1',
      [supermarketId]
    );
    ...
    console.error('Erreur get supermarket dispositifs:', err);
...
router.post('/:supermarketId', authMiddleware, async (req, res) => {
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM supermarket_dispositifs WHERE supermarket_id = $1',
      [supermarketId]
    );
    ...
    if (existing.rows.length > 0) {
      result = await pool.query(
        'UPDATE supermarket_dispositifs SET data = $1, updated_at = CURRENT_TIMESTAMP WHERE supermarket_id = $2 RETURNING *',
        [JSON.stringify(data), supermarketId]
      );
    } else {
      result = await pool.query(
        'INSERT INTO supermarket_dispositifs (supermarket_id, data) VALUES ($1, $2) RETURNING *',
    ...
    console.error('Erreur save supermarket dispositifs:', err);
    res.status(500).json({ message: 'Erreur serveur' });
  }
});
